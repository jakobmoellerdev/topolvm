...
func makeMountOptions(readOnly bool, mountOption *csi.VolumeCapability_MountVolume) ([]string, error) {
	optionSize := 2
	if mountOption != nil {
		optionSize += len(mountOption.MountFlags)
	}

	mountOptions := make([]string, 0, optionSize)
...
func (s *nodeServerNoLocked) nodePublishFilesystemVolume(req *csi.NodePublishVolumeRequest, lv *proto.LogicalVolume) error {
	...
	if !os.IsNotExist(err) || strings.Contains(err.Error(), "no such file or directory") {
		...
		mounted, err = filesystem.IsMounted(lv.GetPath(), req.GetTargetPath())
		if err != nil {
...
func (s *nodeServerNoLocked) nodePublishBlockVolume(req *csi.NodePublishVolumeRequest, lv *proto.LogicalVolume) error {
	...
	_, err = os.Create(req.GetTargetPath())
	if err != nil {
	...
	if err := s.mounter.Mount(lv.GetPath(), req.GetTargetPath(), "", append(mountOptions, "bind")); err != nil {
...
func (s *nodeServerNoLocked) nodeUnpublishBlockVolume(req *csi.NodeUnpublishVolumeRequest) error {
	if err := s.mounter.Unmount(req.GetTargetPath()); err != nil {
		return status.Errorf(codes.Internal, "unmount failed for %s: error=%v", req.GetTargetPath(), err)
	}
